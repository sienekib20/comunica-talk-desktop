...
export class AppData {
	 ...
	 * Persist appData.
	 *
	 * Everything but the credentials goes to LocalStorage. The credentials are
	 * handed to the main process, which stores them encrypted by the OS keychain.
	 */
	persist() {
		const { credentials, ...rest } = this.toJSON()
		localStorage.setItem(this.storageKey, JSON.stringify(rest))
		window.TALK_DESKTOP.setCredentials(credentials)
			.catch((error) => console.error('Error on credentials persist', error))
	}

	/**
	 * Restore appData from LocalStorage and the credentials from the OS keychain
	 *
	 * @return {Promise<AppData>} this
	 */
	async restore() {
		try {
		...
		try {
			...
			console.error('Error on credentials restore', error)
		}
