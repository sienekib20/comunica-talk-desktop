...
const icons = {
	...
	// On macOS in development the app runs from the default Electron executable,
	// so the Dock shows the Electron icon unless it is set explicitly
	dock: require('../../img/icons/icon.png'),
...
/**
 * Get the Dock icon, only required on macOS in development
 *
 * @return {string} Path to the icon
 */
function getDockIcon() {
	return path.join(app.getAppPath(), '.webpack/main', icons.dock)
}
