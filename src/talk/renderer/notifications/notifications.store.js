...
/**
 * Whether the app is locked. While locked the notification content is not shown,
 * the notification itself still arrives.
 */
let isAppLocked = false
window.TALK_DESKTOP.lock.isLocked().then((locked) => {
	isAppLocked = locked
})
window.TALK_DESKTOP.lock.onChange((event, locked) => {
	isAppLocked = locked
})
...
export function createNotificationStore() {
	...
	async function showNativeNotification(notification) {
		...
		if (shouldShowCallPopup) {
			...
			// While locked the notification still arrives, but without its content
			const n = new Notification(isAppLocked ? t('talk_desktop', 'New message') : notification.subject, {
				title: isAppLocked ? t('talk_desktop', 'New message') : notification.subject,
				lang: appData.userMetadata.locale,
				body: isAppLocked ? '' : notification.message,
