...
const TALK_DESKTOP = {
	...
	/**
	 * Get the credentials stored encrypted by the OS keychain
	 *
	 * @return {Promise<{ server: string, user: string, password: string }|null>}
	 */
	getCredentials: () => ipcRenderer.invoke('credentials:get'),
	/**
	 * Store the credentials encrypted by the OS keychain, null removes them
	 *
	 * @param {{ server: string, user: string, password: string }|null} credentials - Credentials
	 * @return {Promise<void>}
	 */
	setCredentials: (credentials) => ipcRenderer.invoke('credentials:set', credentials),
	/**
	 * Lock screen. The app keeps running while locked, so messages and calls still arrive.
	 */
	lock: {
		/**
		 * Lock the app now
		 ...
		 */
		isLocked: () => ipcRenderer.invoke('lock:isLocked'),
		/**
		 * Listen for the app being locked and unlocked
		 *
		 * @param {(event: import('electron').IpcRendererEvent, locked: boolean) => void} callback - Callback
		 */
		onChange: (callback) => ipcRenderer.on('lock:change', callback),
		/**
		 ...
		 *
		 * @return {Promise<boolean>} - Whether the authentication succeeded
		 */
		unlockWithBiometrics: () => ipcRenderer.invoke('lock:unlockWithBiometrics'),
		/**
		 * Log out from the lock screen
		 *
		 * @return {Promise<void>}
		 */
		logout: () => ipcRenderer.invoke('lock:logout'),
	},
	/**
